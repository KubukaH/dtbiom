...
import usrbg from '../../assets/imgs/user-bg.svg';
import blank_avatar from '../../assets/imgs/blank-avatar.jpg';

export const AdminProfileDetails = () => {
  const { user } = useCTX()
  const userbg = new URL(usrbg, import.meta.url);
  const avatar = new URL(blank_avatar, import.meta.url);

  return (
    <article className="w-[95%] mx-auto my-4 h-full rounded-lg border bg-white">
      <div className="relative z-20 h-35 md:h-65">
        <img
          alt="profile cover"
          className="h-40 w-full rounded-tl-sm rounded-tr-sm object-cover object-center"
          src={userbg}
        />
        <div className="absolute bottom-1 right-1 z-10 xsm:bottom-4 xsm:right-4">
          <label
            htmlFor="cover"
            className="flex cursor-pointer bg-indigo-400 items-center justify-center gap-2 rounded bg-primary py-1 px-2 text-sm font-medium text-white hover:bg-opacity-80 xsm:px-4"
          >
      ...
      <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
        <div className="relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
          <div className="relative drop-shadow-2">
            <img alt="profile" src={avatar} className="rounded-full mx-auto absolute -top-20 w-32 h-32 shadow-md border-4 border-white transition duration-200 transform hover:scale-110" />
            <label
              htmlFor="profile"
              className="absolute bg-indigo-400 bottom-0 right-0 flex h-8.5 w-8.5 cursor-pointer items-center justify-center rounded-full bg-primary text-white hover:bg-opacity-90 sm:bottom-2 sm:right-2"
            >
              <svg
                className="fill-current"
                width="14"
        ...
        <div className="mt-4">
          ...
          {user.app_metadata.roles.map((role) => <p className="inline-block font-medium">{role}</p>)}
